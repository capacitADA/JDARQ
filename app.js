...
function modalRecordar(clienteId, equipoId, fecha) {
    ...
    if (esJMC) {
        ...
        if (tienda) {
            ...
            msg = `Hola *${tienda.coordinador}*, recordatorio: activo *${e?.marca} ${e?.modelo}* tienda *${tienda.tienda} (SAP ${sap})* requiere mantenimiento el *${fechaF}*. Confirmar visita. JD Arquisoluciones S.A.S 📞 310 553 3937`;
        } else { tel = c?.telefono; destinatario = c?.nombre; msg = `Hola *${c?.nombre}*, recordatorio: activo *${e?.marca} ${e?.modelo}* requiere mantenimiento el *${fechaF}*. JD Arquisoluciones S.A.S 📞 310 553 3937`; }
    } else { tel = c?.telefono; destinatario = c?.nombre; msg = `Hola *${c?.nombre}*, recordatorio: activo *${e?.marca} ${e?.modelo}* requiere mantenimiento el *${fechaF}*. JD Arquisoluciones S.A.S 📞 310 553 3937`; }
...
function modalQR(eid) {
    ...
    setTimeout(async () => {
        ...
        ctx.fillText('☎  310 553 3937',W/2,s2y+strip2/2+6);
