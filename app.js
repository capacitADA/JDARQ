...
async function generarPDFOrden(s) {
    ...
    try {
        ...
        pdf.rect(M, y, CW, 13);
        if(logoB64) { try{ pdf.addImage(logoB64,'PNG', M+2, y+1.5, 11, 10); }catch(e){} }
        pdf.setFont('helvetica','bold'); pdf.setFontSize(11); pdf.setTextColor(20);
        pdf.text('ORDEN DE TRABAJO MANTENIMIENTO', M + 15 + (CW-15)/2, y+7.5, {align:'center'});
        y += 13;
        ...
        pdf.autoTable({
            startY:y, margin:{left:M,right:M}, tableWidth:CW, theme:'grid',
            styles:{fontSize:6.5, cellPadding:1, minCellHeight:24, lineColor:0, lineWidth:0.25, valign:'middle'},
            ...
            didDrawCell:(data)=>{
                ...
                const cx = data.cell.x+data.cell.width/2, cy = data.cell.y+data.cell.height/2-3, r=6.5;
                if(marcado){ pdf.setDrawColor(192,57,43); pdf.setLineWidth(0.6); pdf.circle(cx,cy,r+1.6); }
                pdf.setDrawColor(0); pdf.setLineWidth(0.35); pdf.circle(cx,cy,r);
                pdf.setFillColor(0); pdf.circle(cx-2.2,cy-1.5,0.5,'F'); pdf.circle(cx+2.2,cy-1.5,0.5,'F');
                pdf.setLineWidth(0.4);
                if(label==='Excelente'){ pdf.line(cx-3,cy+1,cx,cy+2); pdf.line(cx,cy+2,cx+3,cy+1); }
                else if(label==='Bueno'){ pdf.line(cx-2.5,cy+2,cx+2.5,cy+2); }
                else { pdf.line(cx-3,cy+3,cx,cy+2); pdf.line(cx,cy+2,cx+3,cy+3); }
                pdf.setFont('helvetica', marcado?'bold':'normal'); pdf.setFontSize(6.8); pdf.setTextColor(20);
                pdf.text(label+(marcado?' \u2713':''), cx, cy+r+3.2, {align:'center'});
        ...
        pdf.autoTable({
            startY:y, margin:{left:M,right:M}, tableWidth:CW, theme:'grid',
            styles:{fontSize:6.5, cellPadding:1, minCellHeight:24, lineColor:0, lineWidth:0.25, valign:'bottom', halign:'center', textColor:20},
            ...
            didDrawCell:(data)=>{
                ...
                if(data.column.index===0 && firmaTecBase64){
                    try{ pdf.addImage(firmaTecBase64,'PNG', cx-18, data.cell.y+2, 36, 10); }catch(err){}
                }
                if(data.column.index===1 && aprobadoConFirma){
                    try{
                        if(selloBase64) pdf.addImage(selloBase64,'PNG', cx-12, data.cell.y+1.5, 24, 11);
                        if(firmaJefe)   pdf.addImage(firmaJefe,'PNG', cx-13, data.cell.y+5.5, 26, 9);
                    }catch(err){}
                }
            }
        });
